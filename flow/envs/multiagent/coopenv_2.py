...
class CustomSEUYellowSinglePressEnv(MultiEnv):
    """
    A base class for cav-tl
...
#CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/CAV/
...
# trafficlight/trafficlight/trafficlight/trafficlight/trafficlight/trafficlight/trafficlight/trafficlight/trafficlight/
        speeds = []
        dist_to_intersec = []
        acc = []
        NQI_mean = []
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        penalty = len(vel[vel == 0]) / len(vel) * self.num_traffic_lights
...
class CustomSEUYellowPressEnv(MultiEnv):
    """
    A base class for cav-tl
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        self.mapping_out = dict(list(self.mapping_out.items())[:12])
        self.num_traffic_lights = len(self.mapping_inc.keys())
        # used during visualization
        self.observed_ids = []
        # exp setting
    ...
    def get_closest_to_intersection_lane(self, edges, num_closest, padding=True):
        ...
        for veh in all_veh:
            if self.k.vehicle.get_lane(veh)==0:
                ...
                lane_2.append(veh)
        veh_ids_ordered_lane_0=sorted(lane_0, key=self.get_distance_to_intersection)
        for veh in veh_ids_ordered_lane_0:
            if "cav" in veh:
                veh_ids_ordered_lane_0.remove(veh)
        pad_lst = [""] * (num_closest - len(veh_ids_ordered_lane_0))
        result += (veh_ids_ordered_lane_0[:num_closest] + (pad_lst if padding else []))

        veh_ids_ordered_lane_1 = sorted(lane_1, key=self.get_distance_to_intersection)
        for veh in veh_ids_ordered_lane_1:
            if "cav" in veh:
                veh_ids_ordered_lane_1.remove(veh)
        pad_lst = [""] * (num_closest - len(veh_ids_ordered_lane_1))
        result += (veh_ids_ordered_lane_1[:num_closest] + (pad_lst if padding else []))

        veh_ids_ordered_lane_2 = sorted(lane_2, key=self.get_distance_to_intersection)
        for veh in veh_ids_ordered_lane_2:
            if "cav" in veh:
                veh_ids_ordered_lane_2.remove(veh)
        pad_lst = [""] * (num_closest - len(veh_ids_ordered_lane_2))
        result += (veh_ids_ordered_lane_2[:num_closest] + (pad_lst if padding else []))
...
class CustomSEUPressEnv(CustomSEUYellowPressEnv):
    def _apply_rl_actions(self, rl_actions):
        for rl_id, rl_action in rl_actions.items():
            # light
            if rl_id in self.mapping_inc.keys():
...
class CustomSEUSinglePressEnv(CustomSEUYellowSinglePressEnv):
    def _apply_rl_actions(self, rl_actions):
        for rl_id, rl_action in rl_actions.items():
            # light
            if rl_id in self.mapping_inc.keys():
...
class CustomSEUPress_cav_light_Env(CustomSEUPressEnv):
        ...
        for p in ADDITIONAL_ENV_PARAMS_CAVTL.keys():
            if p not in env_params.additional_params:
                raise KeyError(
                    'Environment parameter "{}" not supplied'.format(p))
        self.phases = [0]
        self.num_cav = env_params.additional_params.get("num_cav", 1)
    ...
    def get_state(self):
        ...
        for nodes, edges in self.network.node_mapping:
            ...
            for cav_id in cav_ids:
                ...
                # local_dists_to_intersec.extend([abs(self.k.network.edge_length(
                #     self.k.vehicle.get_edge(cav_id)) -
                #                                     self.k.vehicle.get_position(cav_id)) / max_dist])
    ...
    def get_nearest_light(self,veh_id):
        edgeset = self.network.edges
        edges=[]
        lights=["center0","center1","center2","center3","center4","center5","center6","center7","center8","center9","center10","center11"]
        for edge in edgeset:
            edges.append(edge["id"])

        edge_id = self.k.vehicle.get_edge(veh_id)
        if edge_id not in edges+lights:
            return -1
        if edge_id in lights:
            rl_id_num = int(edge_id.split("center")[ID_IDX])
            return rl_id_num
        for edge in edgeset:
            if edge['id'] == edge_id:
                current_intersection = edge["to"]
                if current_intersection not in lights:
                    return -1
                else:
                    rl_id_num = int(current_intersection.split("center")[ID_IDX])
                    return rl_id_num

class CustomSEUPress_cav_light_DQN_Env(CustomSEUPress_cav_light_Env):
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        super().__init__(env_params, sim_params, network, simulator)
    ...
    def _apply_rl_actions(self, rl_actions):
        for rl_id, rl_action in rl_actions.items():
            ...
            if rl_id in self.mapping_inc.keys():
                ...
                high=abs(self.env_params.additional_params["max_decel"])
                action=low+((rl_action-25)-(-25))/(25-(-25))*(high-low)
                j = int(rl_id.split("cav_")[ID_IDX])
                self.acc[j].append(rl_action)
                self.k.vehicle.apply_acceleration(rl_id, action)
        self.nowsecond.append((self.nowsecond[-1] + 1))

class CustomSEUPress_DQN_Env(CustomSEUPressEnv):
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        super().__init__(env_params, sim_params, network, simulator)
